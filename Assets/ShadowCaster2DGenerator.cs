...
public class ShadowCaster2DCreator : MonoBehaviour
{
	...
	public void AltCreate()
    {
		DestroyOldShadowCasters();
		tilemapCollider = GetComponent<CompositeCollider2D>();

		PhysicsShapeGroup2D physicsShape = new();
		tilemapCollider.GetShapes(physicsShape);
		Debug.Log($" :: {physicsShape.shapeCount}");

		for (int i = 0; i < physicsShape.shapeCount; i++)
		{
			List<Vector2> pathVertices = new();
			physicsShape.GetShapeVertices(i,pathVertices);
			GameObject shadowCaster = new("shadow_caster_" + i);
			shadowCaster.transform.parent = gameObject.transform;
			ShadowCaster2D shadowCasterComponent = shadowCaster.AddComponent<ShadowCaster2D>();
			shadowCasterComponent.selfShadows = this.selfShadows;

			Vector3[] testPath = new Vector3[pathVertices.Count];
			for (int j = 0; j < pathVertices.Count; j++)
			{
				testPath[j] = pathVertices[j];
			}
